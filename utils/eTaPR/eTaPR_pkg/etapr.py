from utils.eTaPR.eTaPR_pkg import tapr
import math
import argparse
from utils.eTaPR.eTaPR_pkg.DataManage import File_IO, Time_Plot
import numpy as np
from utils.eTaPR.eTaPR_pkg.DataManage import Range as rng

class eTaPR(tapr.TaPR):
    def __init__(self, theta_p, theta_r, delta=0.0):
        super(eTaPR, self).__init__(0.0, 0)
        self._predictions_weight = []
        self._predictions_total_weight = 0.0 + 1e-7
    ...
    def eTaR_p(self) -> float:
        scores = self._etar_p()
        return np.mean(scores) # scores.mean()
